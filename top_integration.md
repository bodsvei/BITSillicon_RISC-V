...
|---|-------|-------|----------|
| 1 | ID/EX Pipeline Register — confirm owner; Anirudh or Vivaan? | Anirudh | Before Phase 2 |
| 2 | MEM/WB Pipeline Register — confirm owner; Anirudh or Soham? | Anirudh | Before Phase 2 |
| 3 | Write-then-read forwarding: in `reg_file` or `hazard_unit`? | Anirudh + Dev | Before Phase 2 |
| 4 | JAL vs JALR target computation — in `branch_unit` or `id_stage`? | Anirudh + Aadi | Before Phase 2 |
| 5 | Memory initialization format: `.hex` file via `$readmemh`? | Krishna + Parhawk | Before testbench |
| 6 | Simulation tool: Verilator or Icarus Verilog? | All | ASAP |
